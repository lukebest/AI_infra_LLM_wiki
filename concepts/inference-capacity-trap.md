...
updated: 2026-06-15
type: concept
tags: [inference, reasoning, capacity-trap, kv-cache, serving, parallelism]
...
Capacity Trap 是**机制**（为什么 throughput 崩溃），[[reasoning-cliff]] 是**现象**（KV cache 何时超过 HBM 容量）。Reasoning workload 因 OSL ≫ 10k 将 cliff 提前，有时甚至在 prefill 阶段就饱和。
